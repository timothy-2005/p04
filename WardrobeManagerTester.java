...
public class WardrobeManagerTester {
  ...
  public static boolean testClothingWear() {
    ...
    try {
      ...
      // check if the exception has a message
      if (e.getMessage() == null || e.getMessage().isBlank()) // if there is NO message it's a
  ...
  public static boolean testAddClothing() {
    ...
    if (w.capacity() != 10)
      return false; // if the capacity is not doubled return false
    if (w.size() != 6)
      return false; // if the size is not 6 return false
    // passed all the tests!
    return true;
  }
  ...
  public static boolean testGetClothing() {
    ...
    w.addClothing(c); // add the Clothing object to the Wardrobe
    Clothing compareClothing = w.getClothing("BLACK JEANS", "LEVI"); // compare the Clothing object
                                                                     // from the Wardrobe(case
                                                                     // insensitive)
    Clothing compareClothing2 = w.getClothing("black jeans", "Levi"); // get the Clothing object
                                                                      // from the Wardrobe
    if (!compareClothing.equals(c))
      return false; // if the Clothing object is not the same return false
    if (!compareClothing2.equals(c))
      return false; // if the Clothing object is not the same return false
    // passed all the tests!
    return true;
  }
